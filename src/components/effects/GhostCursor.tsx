...
const GhostCursor = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('GhostCursor: WebGL not available');
      return;
    }
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        iPrevMouse: { value: trailBufRef.current.map((v: THREE.Vector2) => v.clone()) },
        iOpacity: { value: 1.0 },
        iScale: { value: 1.0 },
    ...
    try {
      ...
      console.warn('GhostCursor: postprocessing failed, falling back to direct rendering', e);
      useComposer = false;
    }
    ...
    const resize = () => {
      ...
      if (useComposer && composer) composer.setSize(cssW, cssH);
